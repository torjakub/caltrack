...
export function searchFoodsRemote(query: string): Promise<FoodOut[]> {
  return apiFetch<FoodOut[]>(`/api/v1/foods/search?q=${encodeURIComponent(query)}`);
}

export function lookupBarcodeRemote(barcode: string): Promise<FoodOut> {
...
export function createCustomFoodRemote(payload: CustomFoodInput): Promise<FoodOut> {
