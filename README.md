...
LLM features (OCR nutrition-label scanning, meal review, daily/weekly nutrient analysis) are entirely optional and pluggable — `none` (default), `anthropic`, `openai`, or `ollama` (point at a local model on your network). See [`docs/llm-providers.md`](docs/llm-providers.md).
...
Early development — see the project board / milestones for current progress. Not yet ready for production use.
